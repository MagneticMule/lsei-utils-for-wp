...
class CustomPostManager {
   ...
   * @$screens defaults to 'newsitems' however
   */
  public function __construct( $s = array ( 'newsitems' ) )
  {
    $this->screens = $s;
  }
  ...
  {
    remove_meta_box( 'pageparentdiv', 'newsitems', 'side' );
    // for some odd reason this doesn't always work. Very annoying.
    remove_meta_box( 'postimagediv', 'newsitems', 'side' );
  }
  ...
  **/
  function buildCustomPostWidgets()
  {
    ...
    // add_meta_box( 'cover_image_meta_box', 'Add an Image', array($this,'selectImageMetaBox'), 'newsitems', 'normal', 'high' );
    // add_meta_box( 'cover_sound_meta_box', 'Add a Sound', array($this,'selectSoundMetaBox'), 'newsitems', 'normal', 'high' );
  }
